"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.TbCommentsModule = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const tb_comments_service_1 = require("./tb_comments.service");
const tb_comments_controller_1 = require("./tb_comments.controller");
const tb_comment_entity_1 = require("./entities/tb_comment.entity");
const tb_new_entity_1 = require("../tb_news/entities/tb_new.entity");
const tb_news_module_1 = require("../tb_news/tb_news.module");
const tb_student_entity_1 = require("../tb_student/entities/tb_student.entity");
const tb_student_module_1 = require("../tb_student/tb_student.module");
const tb_admin_entity_1 = require("../tb_admin/entities/tb_admin.entity");
const tb_lecturer_entity_1 = require("../tb_lecturer/entities/tb_lecturer.entity");
const tb_lecturer_module_1 = require("../tb_lecturer/tb_lecturer.module");
const tb_admin_module_1 = require("../tb_admin/tb_admin.module");
let TbCommentsModule = class TbCommentsModule {
};
exports.TbCommentsModule = TbCommentsModule;
exports.TbCommentsModule = TbCommentsModule = __decorate([
    (0, common_1.Module)({
        imports: [
            typeorm_1.TypeOrmModule.forFeature([tb_comment_entity_1.TbComments, tb_new_entity_1.TbNew, tb_student_entity_1.TbStudent, tb_admin_entity_1.TbAdmin, tb_lecturer_entity_1.TbLecturer]),
            tb_student_module_1.TbStudentModule,
            tb_lecturer_module_1.TbLecturerModule,
            tb_admin_module_1.TbAdminModule,
            tb_news_module_1.TbNewsModule,
        ],
        providers: [tb_comments_service_1.TbCommentsService],
        controllers: [tb_comments_controller_1.TbCommentsController],
        exports: [tb_comments_service_1.TbCommentsService],
    })
], TbCommentsModule);
//# sourceMappingURL=tb_comments.module.js.map